...
sf_count_t at_audio_processor (SNDFILE *infile, SNDFILE *outfile)
{
	...
	int window_size = 0, fft_size = 0, pa_error;
    ...
    noverlap = floor( window_size * at_get_overlap() / 100);
    ...
    audio_container_t *audio_data_fft = at_allocate_buffer (at_get_out_channels(), fft_size, input_samplerate);
    ...
    do {
			if (frames_read == 0)
			{
				if ((count = sf_readf_double (infile, multi_data, window_size)) <= 0)
					exit (1);
				memcpy((void *) prev_multi_data, (void *) (multi_data + nslide * info.channels), sizeof(* multi_data) * noverlap * info.channels);
			}
			else
			{
				count = sf_readf_double (infile, (multi_data + noverlap * info.channels), nslide);
			  ...
			  if (at_get_out_file())
				  sf_writef_double (outfile, multi_data, nslide);
			  else
			  {
...
audio_container_t* at_allocate_buffer (int channels, int size, int samplerate)
{
	if (size <= 0)
	{
	...
	if (input_channels == 5)
	{
		for (int i = 0; i < container->length; i++)
		{
			container->channel[LFE][i] = (container->channel[FL][i] + container->channel[FR][i] +
					container->channel[C][i] + container->channel[SL][i] + container->channel[SR][i]) / 5.0;
		}
...
void at_create_lfe(audio_container_t *container, int sampling_freq, int fft_size)
{
	...
	double *buffer = init_buffer_dbl(fft_size);
	...
	int n_freq = ceil ((2 * CUTOFF_FREQ * ((fft_size / 2.0) - 1) / sampling_freq));
